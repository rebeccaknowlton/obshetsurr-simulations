...
for (i in 1:num.sim) {
  ...
  if(i == 2) {na.hold = rep(NA, length(output))}
  if(i == 1) {
  	...
  	output <- tryCatch(
  {
    ...
    out = obs.het.surr(df.train = data.train, df.test = data.test, type = "all", var.want = TRUE, threshold = k)
    out  # return value if successful
  },
  error = function(e) {
    # What to do if an error occurs
    message("Error caught: ", e$message)
    return(na.hold)  # return NULL or some default value
  }
)
  }
