...
boot.var <- function(df.train, df.test, type, numeric_predictors, categorical_predictors, threshold, use.actual.control.S, gam.smoothers = NULL, tree.tuners =NULL, num.boot = 500) {
  ...
  if (type %in% c("linear", "gam", "trees")) {
    ...
    df.test$delta.var.trees <- NA
    df.test$delta.s.var.trees <- NA
    df.test$R.s.var.trees <- NA
    df.test$delta.lower.trees <- NA
    df.test$delta.upper.trees <- NA
    df.test$delta.s.lower.trees <- NA
    df.test$delta.s.upper.trees <- NA
    df.test$R.s.lower.trees <- NA
    df.test$R.s.upper.trees <- NA
  }
