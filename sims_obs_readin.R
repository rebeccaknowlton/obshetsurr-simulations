...
outputfile=c()
for(u in 1:20) {
	out = read.table(paste("obs_outputfile", setting, "_011725_",u,".txt", sep=""), header = T)
	out = out[!is.na(out[,1]),]
  outputfile = rbind(outputfile,out)
}
...
colnames(flag.results) <- c("Linear", "GAM")
...
plot_data_Rs$Method <- factor(plot_data_Rs$Method, levels = c("Linear", "GAM"))
