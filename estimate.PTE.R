estimate.PTE <- function (df.train, df.test, type, numeric_predictors, categorical_predictors, use.actual.control.S, gam.smoothers = NULL, tree.tuners = NULL, want.smooth = FALSE, want.tune = FALSE) {
  ...
  if (type %in% c("linear", "gam", "trees")) {
    ...
    df.test$delta.trees <- NA
    df.test$delta.s.trees <- NA
    df.test$R.s.trees <-NA
  }
